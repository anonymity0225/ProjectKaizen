...
class TransformationRegistry:
    ...
    def get_summary(self) -> Dict[str, Any]:
        """Get a summary of all applied transformations (thread-safe)."""
        with self._lock:
            return {
...
def _ensure_nltk_data(resource: str, download_name: str = None) -> None:
...
def extract_date_components(
    ...
    metadata = _create_success_metadata(
        ...
        affected_columns=[column] + added_columns
    )
...
def tokenize_text(
    ...
    _ensure_nltk_data('tokenizers/punkt')
    _ensure_nltk_data('corpora/stopwords')
    
    if method == "lemmatization":
        _ensure_nltk_data('corpora/wordnet')
...
def apply_tfidf_vectorization(
    ...
    metadata = _create_success_metadata(
        ...
        added_columns=feature_names,
        dropped_columns=[column],
        affected_columns=feature_names,
        additional_info={
    ...
    transformation_registry.add_operation(metadata)
    logger.info(f"Successfully applied TF-IDF vectorization to column '{column}', created {len(feature_names)} features")
...
def apply_pca(
    ...
    transformation_registry.add_operation(metadata)
    logger.info(f"Successfully applied PCA to {len(columns)} columns, created {n_components} components")
    return df_result, metadata
...
def apply_multiple_transformations(
    ...
    for i, transform_config in enumerate(transformations):
        ...
        try:
            ...
            # Continue with remaining transformations instead of stopping
    
    logger.info(f"Batch transformation completed. Applied {len(transformations)} transformations, "
                f"{sum(1 for m in all_metadata if m['status'] == 'success')} successful")
